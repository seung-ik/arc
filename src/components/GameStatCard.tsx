...
import { GameType, GameStat, GAME_NAMES, GAME_ICONS } from '@/constants/gameTypes';
...
const Card = styled.div<{ $isActive: boolean }>`
  background: ${(props) =>
    props.$isActive ? props.theme.colors.background : props.theme.colors.backgroundGray};
  border: 1px solid
    ${(props) => (props.$isActive ? props.theme.colors.border : props.theme.colors.borderLight)};
  border-radius: 12px;
  padding: 16px;
  display: flex;
...
const GameName = styled.h3<{ $isActive: boolean }>`
  ...
  color: ${(props) =>
    props.$isActive ? props.theme.colors.textBlack : props.theme.colors.textLightGray};
  margin: 0;
  text-align: center;
`;
...
const Percentile = styled.div<{ $isActive: boolean }>`
  ...
  color: ${(props) =>
    props.$isActive ? props.theme.colors.textGray : props.theme.colors.textLightGray};
  text-align: center;
`;
...
export default function GameStatCard({ gameType, gameStat }: GameStatCardProps) {
